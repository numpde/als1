...
```{r, dpi=96, out.extra='class="zoomable scrollable"', results='hide'}
datasets$ab %>%
  take_rows(get_goi_genes()) %>%
  `[`(, .$celltype == "374_VLMC") %>%
  coex() %>%
...
```{r, out.extra='class="zoomable scrollable"', results='hide', dpi=144}
datasets$bh %>%
  take_rows(get_goi_genes()) %>%
  coex() %>%
...
```{r, out.extra='class="zoomable scrollable"', results='hide', dpi=96}
datasets$bh %>%
  take_rows(get_goi_genes()) %>%
  `[`(, .$celltype == "FB1") %>%
  coex() %>%
