...
show_coex_graph <- function(coex_df, de_table, n = 77) {
  ...
  ex_graph <-
    ...
    mutate(
      color = colorRamp(c("blue", "white", "red"))(color) %>% RGB(alpha = 0.2)
    ) %>%
    ...
    igraph::graph_from_data_frame(
      ...
      vertices = (
        de_table %>%
          ...
          select(name = symbol, color, lfc)
      )
    )
  
  igraph::E(ex_graph)$consistent <- 
    list(
      edges = ex_graph %>% igraph::as_data_frame("edges"),
      v_lfc = ex_graph %>% igraph::as_data_frame("vertices") %>% pull(lfc, name)
    ) %>%
      with({
        edges %>%
          mutate(consistent = (sign(v_lfc[from] * v_lfc[to]) == sign(w))) %>%
          pull(consistent)
      })
        ...
        function(slide) {
          ggplot() +
            ...
            geom_segment(
              ...
              size = 0.1,
              linetype = if_else(igraph::E(ex$graph)$consistent, "solid", "dashed")
            ) +
            # TF-target curves
            geom_curve(
              data = tf$segment,
