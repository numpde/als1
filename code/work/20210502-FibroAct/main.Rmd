...
show_coex_graph <- function(coex_df, de_table, n = 77) {
  ...
  # The interaction graph contains ligand-receptor; drop those
  # "Cannot use an edge sequence from another graph":
  #graphs$ia %<>% igraph::delete_edges(igraph::E(igraph::intersection(graphs$lr, graphs$ia)))
...
```{r, out.extra='class="zoomable scrollable"', results='hide'}
datasets$ab %>%
  take_rows(get_goi_genes()) %>%
  coex() %>%
