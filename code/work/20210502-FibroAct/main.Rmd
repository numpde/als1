...
show_coex_graph <- function(coex_df, de_table, n = 77) {
  ...
  graphs <- list(
    ...
    lapply(function(df) {
      df %>%
        ...
        group_by(a, b) %>% summarize(W = sum(W), .groups = "keep") %>%
        filter(W != 0) %>%
  ...
  graphs %>%
    ...
    with({
      list(
        slide1 = list(
          alpha = list(ex = 2e-2, tf = 0.80, lr = 3e-2, ia = 3e-2),
          title = "TF -> target"
        )
        , slide2 = list(
          alpha = list(ex = 2e-2, tf = 3e-2, lr = 0.80, ia = 3e-2),
          title = "Ligand -> receptor"
        )
        , slide3 = list(
          alpha = list(ex = 2e-2, tf = 3e-2, lr = 3e-2, ia = 0.80),
        ...
        function(slide) {
          ggplot() +
            ...
            geom_segment(
              ...
              size = 0.1,
              linetype = if_else(igraph::E(ex$graph)$coherent, "solid", "dashed")
            ) +
            # TF-target curves
            geom_curve(
              data = tf$segment,
              ...
              arrow = arrow(
                ...
                angle = if_else(igraph::E(tf$graph)$W > 0, 10, 90)
              ),
              color = igraph::E(tf$graph)$color,
              alpha = slide$alpha$tf,
              size = 0.2,
              curvature = 0.05,
              linetype = if_else(igraph::E(tf$graph)$coherent, "solid", "dashed")
            ) +
            ...
            geom_curve(
              ...
              arrow = arrow(
                length = unit(0.007, "npc"), 
                angle = if_else(igraph::E(lr$graph)$W > 0, 10, 90)
              ),
              color = igraph::E(lr$graph)$color,
              alpha = slide$alpha$lr,
              size = 0.2,
              curvature = 0.05,
              linetype = if_else(igraph::E(lr$graph)$coherent, "solid", "dashed")
            ) +
            ...
            geom_curve(
              ...
              arrow = arrow(
                length = unit(0.007, "npc"), 
                angle = if_else(igraph::E(ia$graph)$W > 0, 10, 90)
              ),
              color = igraph::E(ia$graph)$color,
              alpha = slide$alpha$ia,
              size = 0.2,
              curvature = 0.05,
              linetype = if_else(igraph::E(ia$graph)$coherent, "solid", "dashed")
            ) +
            # Labels
            geom_text(
              data = ex$pos,
