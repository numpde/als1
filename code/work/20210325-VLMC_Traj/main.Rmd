...
and
metadata from
[here](https://cells.ucsc.edu/?ds=oligo-lineage-ms).
...
```{r}
sce_abm_ch@colData %<>%
  cbind(sce_abm_ch.slingshot %>% slingshot::slingPseudotime())
```
...
We suppose
the collection of VLMC cells 
is representative of a developmental trajectory.
There could be a fork,
but let's partition the samples 
into "early" (first quarter) and "late" (last quarter)
in terms of pseudotime,
and find DE genes.
From the PCA figure, 
these might correspond to 
two leaves of the development tree.
...
```{r}
sce_abm_ch$cell_type_alias_label %>%
  unique() %>%
  sort() %>%
  sapply(function(ct) {
    sce_abm_ch %>% {
      ii <- ((.)$cell_type_alias_label == ct)
      c(
  ...
  }) %>%
  {
    pheatmap::pheatmap(
      ...
      annotation_col = (
        (.)@colData %>%
          as.data.frame() %>%
          select(subclass_label, cell_type_alias_label, Lineage1)
      )
    )
  }
```
...
for FB1 and FB2 from the Betsholtz dataset.
...
Look at the same genes
for VLMC from the Castelo-Branco dataset.

```{r}
# Columns of A against columns of B
# A and B are SingleCellExperiment-like
cosine <- function(A, B) {
  A <- A@assays@data$logcounts
  B <- B@assays@data$logcounts
...
a_vs_b <- cosine(sce_abm_ch, sce_cb)

sce_abm_ch$cell_type_alias_label[sapply(a_vs_b %>% as.data.frame, which.max)] %>% table
```
