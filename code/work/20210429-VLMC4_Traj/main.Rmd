---
title: "VLMC cells from C-B, 2018"
...
If A and B are SingleCellExperiment-like,
then `cosine` computes
the cosine similarity of
of samples of A against samples of B.
...
### Castelo-Branco, 2018 {.tabset .tabset-fade}
...
sce_cb <-
  make_sce(
    counts = (
      path_to("*/CasteloBranco-2018/*/expr.csv.gz") %>%
        from_csv()
    ),
    coldata = (
      path_to("*/CasteloBranco-2018/*/meta.csv.gz") %>%
        from_csv()
    )
  ) %>%
...
sce_cb@assays@data$counts %<>% as.matrix()
```

```{r, include=FALSE}
# Test
sce_cb %>% DESeq2::DESeqDataSet(design = ~celltype)
```
...
sce_abm_ch@colData$celltype = sce_abm_ch@colData$cell_type_alias_label
```
...
## Analysis of "Castelo-Branco, 2018"
...
### VLMC3 {.tabset .tabset-fade}

We first look at what differentiates
the VLMC3 cluster.
...
de_results.vlmc3 <-
  ...
  {
    (.) %>%
      DESeq2::DESeqDataSet(design = ~celltype) %>%
...
Log2 fold-change is relative to VLMC3.
Hence, *negative l2fc* means *up in VLMC3*.
...
sce_cb %>%
  list(
    sce = .,
    top = de_results.vlmc3 %>%
      slice_min(pvalue, n = 44) %>%
      arrange(log2FoldChange)
  ) %>%
  with({
    print(paste("Min abs log2 fold-change:", round(min(abs(top$log2FoldChange)), 2)))
...
```{r}
# Number of genes for GO analysis
n <- 33
```
...
enriched_go_terms <-
  ...
  {
    list(
      up_vlmc3 = (.) %>% filter(log2FoldChange < 0),
      up_other = (.) %>% filter(log2FoldChange > 0)
    )
  } %>%
  lapply(function(.) {
    (.) %>%
  ...
  within({
    ...
    print(paste("Up in VLMC3:", f(up_vlmc3), "..."))
    print(paste("Up in other:", f(up_other), "..."))
    rm(f)
  }) %>%
...
GO terms associated with `r n` "VLMC3" genes.

```{r}
enriched_go_terms$up_vlmc3 %>% show_go_table()
```

#### GO up in other

GO terms associated with `r n` "other" genes.

```{r}
enriched_go_terms$up_other %>% show_go_table()
```


### Ctrl vs EAE (restricted) {.tabset .tabset-fade}

Now we look at 
Ctrl vs EAE cells
**only within the "Other" (non-VLMC3) cluster**,
...
de_results.group <-
  ...
  {
    (.) %>%
      DESeq2::DESeqDataSet(design = ~Group) %>%
      DESeq2::estimateSizeFactors() %>%
      DESeq2::DESeq(quiet = TRUE, fitType = "local") %>%
      DESeq2::results()
  } %>%
...
Log2 fold-change is relative to the Ctrl group.
Hence, *positive l2fc* means *up in EAE*.
...
sce_cb %>%
  ...
  list(
    sce = .,
    top = de_results.group %>%
      slice_min(pvalue, n = 55) %>%
      arrange(log2FoldChange)
  ) %>%
  with({
    print(paste("Min abs log2 fold-change:", round(min(abs(top$log2FoldChange)), 2)))
...
enriched_go_terms <-
  ...
  {
    list(
      up_ctrl = (.) %>% filter(log2FoldChange < 0),
      up_eae = (.) %>% filter(log2FoldChange > 0)
    )
  } %>%
  lapply(function(.) {
    (.) %>%
  ...
  within({
    f <- (function(x) x[1:5] %>% paste(collapse = ", "))
    print(paste("Up in Ctrl:", f(up_ctrl), "..."))
    print(paste("Up in EAE: ", f(up_eae), "..."))
    rm(f)
  }) %>%
  lapply(function(de_results) {
    de_results %>%
...
GO terms associated with `r n` genes up in the Ctrl group.

```{r}
enriched_go_terms$up_ctrl %>% show_go_table()
```

#### GO up in EAE

GO terms associated with `r n` genes up in the EAE group.

```{r}
enriched_go_terms$up_eae %>% show_go_table()
```
...
This comparison depends a lot on
the subset of genes along which to compare.
...
list(
  ...
  g = de_results.group %>% slice_min(pvalue, n = 150) %>% rownames()
) %>%
  with({
    # Keep only the common genes
    g <- intersect(g, intersect(rownames(a), rownames(b)))
  
    pheatmap::pheatmap(
      cosine(a[g, ], b[g, ]) %>% as.data.frame(),
      show_colnames = FALSE,
      show_rownames = FALSE,
      cluster_cols = TRUE,
      cluster_rows = TRUE,
...
```{r}
sce_cb@colData %<>%
  cbind(sce_cb.slingshot %>% slingshot::slingPseudotime())
```
